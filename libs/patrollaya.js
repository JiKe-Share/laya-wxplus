...
(function (exports) {
	...
	if(_.contains==null) _ = require("./underscore-min");

//	if(Laya.Browser.onWeiXin) _ = require("./underscore-min");
	//_ = require("./underscore-min");
	mesh= window.Laya;
	...
	var computeCentroids = function (geometry) {
		...
		for ( f = 0, fl = geometry.faces.length; f < fl; f ++ ) {
			...
			face.centroid=add(face.centroid,geometry.vertices[ face.a ]);
			face.centroid=add(face.centroid,geometry.vertices[ face.b ]);
			face.centroid=add(face.centroid,geometry.vertices[ face.c ]);
			face.centroid=scale(face.centroid,1/3);
...
//交叉检测
			...
			if (distanceToSquared(polygon.centroid,navigationMesh.polygons[i].centroid) > 100 * 40) continue;
	...
	var buildNavigationMesh = function (geometry) {
		...
		geometry.CombinePointPositions();
	...
	_.extend(exports, {
		...
		getRandomNode: function (zone, group, nearPosition, nearRange) {
			...
			if (!zoneNodes[zone]) return new Laya.Vector3();
			...
			return _.sample(candidates) || new Laya.Vector3();
		},
		findPath: function (startPosition, targetPosition, zone, group) {
			...
			var threeVectors = [];
			var last=new Laya.Vector3();
			var isSame=false;

			_.each(channel.path, function (c) {
				var vec = new Laya.Vector3(c.x, c.y, c.z);
				if(vec.x.toFixed(2)!=last.x.toFixed(2)||
				vec.y.toFixed(2)!=last.y.toFixed(2)||
				vec.z.toFixed(2)!=last.z.toFixed(2)){
			...
			threeVectors.shift();
			if(isSame&&threeVectors.length>1){
				threeVectors.shift();
			}
